...
def basic_case_dir(testdir):
    ...
    case_dir.join('snapshot1.bin').write_binary(b'Random Test Data')
    return case_dir


def test_assert_match_with_external_snapshot_path(testdir, basic_case_dir):
    testdir.makepyfile("""
        from pytest_snapshot.custom import file_match
        from pathlib import Path

        def test_sth(snapshot):
            snapshot.snapshot_dir = 'case_dir'
            snapshot.assert_match('test_assert_match_with_external_snapshot_path.bin', Path('not_case_dir/snapshot1.bin').absolute(), file_match)
    """)
    testdir.makefile('.bin', b'Random Test Data')
    result = testdir.runpytest('-v')
    result.stdout.fnmatch_lines([
        '*::test_sth FAILED*',
        "E* AssertionError: Snapshot path not_case_dir?snapshot1.bin is not in case_dir",
    ])
    assert result.ret == 1
...
def test_assert_match_success(testdir, basic_case_dir):
    testdir.makepyfile("""
        ...
        def test_sth(snapshot):
            snapshot.snapshot_dir = 'case_dir'
            snapshot.assert_match('test_assert_match_success.bin', 'snapshot1.bin', file_match)
    """)
    testdir.makefile('.bin', b'Random Test Data')
...
def test_assert_match_failure(testdir, basic_case_dir):
    testdir.makepyfile("""
        ...
        def test_sth(snapshot):
            snapshot.snapshot_dir = 'case_dir'
            snapshot.assert_match('test_assert_match_failure.bin', 'snapshot1.bin', file_match)
    """)
    testdir.makefile('.bin', b'Random INCORRECT Test Data')
    result = testdir.runpytest('-v')
    result.stdout.fnmatch_lines([
        '*::test_sth FAILED*',
        ">* raise AssertionError(snapshot_diff_msg)",
        'E* AssertionError: value does not match the expected value in snapshot case_dir?snapshot1.bin',
        "E* assert False",
        "E* +  where False = <function cmp at *",
        "E* +    where <function cmp at *",
    ])
    assert result.ret == 1

def test_assert_match_with_args_success(testdir, basic_case_dir):
    testdir.makepyfile("""
        def func(snapshot, v1, v2, a1, k1):
            assert v1 == v2
            assert a1 == "a1"
            assert k1 == "k1"
        ...
        def test_sth(snapshot):
            snapshot.snapshot_dir = 'case_dir'
            snapshot.assert_match('the valuÉ of snapshot1.txt', 'the valuÉ of snapshot1.txt', func, "a1", k1="k1")
    """)
    assert_pytest_passes(testdir)
